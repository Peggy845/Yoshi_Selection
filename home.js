...
function createSubCategoryMenu(mainCat, subCategories) 
{
  ...
  all.onclick = () => 
  {
    window.location.href = `product-list.html?main=${encodeURIComponent(mainCat)}&sub=全部`;
  };
  ...
  subCategories.forEach(name => 
  {
	...
	{
      // 跳轉並在網址帶上子分類名稱
	  window.location.href = `product-list.html?main=${encodeURIComponent(mainCat)}&sub=${encodeURIComponent(name)}`;
    };
